...
class Cube {
    ...
    for (let i = 0; i < this.order; i++) {
      for (let j = 0; j < this.order; j++) {
        for (let k = 0; k < this.order; k++) {
          ...
          const geometryFace = new THREE.PlaneGeometry(
            this.pieceSize * 0.85,
            this.pieceSize * 0.85
          );
  ...
  // Ketika berhasil menyelesaikan cube
  showCongrats = () => {
  ...
  rotateSclice = (axis, index, direction, del = false) => {
    ...
    switch (axis) {
      case "x":
        ...
        for (let i = 0; i < this.order; i++) {
          for (let j = 0; j < this.order; j++) {
            if (del) scene.remove(this.blocks[index][i][j].piece);
      ...
      case "y":
        ...
        for (let i = 0; i < this.order; i++) {
          for (let j = 0; j < this.order; j++) {
            if (del) scene.remove(this.blocks[i][index][j].piece);
            // Backing up
            if (!tempSclice["" + i + j])
      ...
      case "z":
        ...
        for (let i = 0; i < this.order; i++) {
          for (let j = 0; j < this.order; j++) {
            if (del) scene.remove(this.blocks[i][j][index].piece);
            // Backing up
            if (!tempSclice["" + i + j])
